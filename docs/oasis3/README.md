...
WUSTL Knight ADRC 의 ~30년 retrospective 통합 longitudinal multi-modal 코호트. **1,379 subjects** (cognitively normal 755 + cognitive decline 622 + 2025-12 Tau 통합 시 +1; 원본 release N=1,378), 42–95세, MRI + PET + clinical/cognitive 통합 배포. Tau (AV1451) sub-project 두 개 (`OASIS-3_AV1451` n=451 cross-sectional + `OASIS-3_AV1451_Longitudinal` n=85)도 2025-12부터 본 프로젝트에 통합. 임상 데이터는 NACC UDS v2/v3 mix 표준이며, 폼 컬럼 정의는 NACC와 공유 (`docs/_shared/nacc_uds_forms.md`). 본 폴더는 OASIS3 cohort-specific quirk (file 이름, OASISID 그래머, days_to_visit anchor, USDa3 typo, c1=`psychometrics` token 등) 에 집중한다.
...
| 항목 | 값 |
|------|---|
| Subject 키 | `OASISID` (`OAS3xxxx`, 1-padded 4 digits, 1379까지) |
...
| Subject 수 | **1,379** (CN 755 + cognitive decline 622 + 2025-12 Tau 통합 시 +1; 원본 release N=1,378) |
| Visit 행수 (UDS b4) | 8,627 (CDR 폼 기준) |
| Visit 행수 (UDS 일반) | 8,500 (대부분 폼) |
| Optional 폼 행수 | 4,090 (a3 가족력, b3 UPDRS) |
| 등록·추적 기간 | ~30년 retrospective (multiple Knight ADRC 연구 통합, **신규 환자 모집 없음**) |
| 연령 범위 | 42 – 95세 |
| 모달리티 | T1 / T2 / FLAIR / SWI / dMRI / rsfMRI / Amyloid PET (PIB, AV45) / Tau PET (AV1451) / FDG PET |
| Sub-projects | **OASIS-3_AV1451** (Tau xs, n=451) + **OASIS-3_AV1451_Longitudinal** (Tau long, n=85, xs의 subset) — 2025-12 본 프로젝트 통합 |
| UDS 버전 | v2 ↔ v3 mix (FORMVER 컬럼 없음 — 검사 셋 / 시점 추정) |
| Single ADRC | WUSTL Knight ADRC 단일 사이트 |
| 데이터 접근 | DUA + Research Statement (Aims/Methods/Variables of Interest) 동의 후 통합 [Data Access Form](https://sites.wustl.edu/oasisbrains/home/access/) → 승인 후 [NITRC-IR (XNAT)](https://www.nitrc.org/ir/)에서 [`oasis-scripts`](https://github.com/NrgXnat/oasis-scripts)로 다운로드. 절차: [`protocol.md`](protocol.md) §7. |
...
| [`protocol.md`](protocol.md) | Knight ADRC 배경, OASIS 시리즈 비교 (sub-project AV1451/AV1451L 가족 포함), NACC UDS v2/v3, 모달리티 / PET 트레이서, **데이터 access 절차 (§7) + 다운로드 도구 (NrgXnat/oasis-scripts) + Avid 사전 검토** — 연구 맥락 이해 + 데이터 받기 시작할 때 |
...
11. **Avid Radiopharmaceuticals 사전 검토 의무.** AV1451 (Flortaucipir) 또는 AV45 (Florbetapir) 데이터를 사용한 모든 출판/발표 (peer-reviewed paper, preprint, poster, oral 포함)는 **출판 30일 전 Avid에 사전 검토 제출 필수** (Eli Lilly 자회사, 지적재산권 검토). 위반 시 OASIS access 박탈. cross-sectional/longitudinal 동일 적용. 자세한 내용은 [`protocol.md`](protocol.md) §7.4.
12. **NIH NOT-OD-25-083 국가 제한.** OASIS-3 / OASIS-4 (NIH Controlled-Access)는 중국(홍콩/마카오 포함), 러시아, 이란, 북한, 쿠바, 베네수엘라 소재 연구자/기관 access 차단. OASIS-1 / OASIS-2는 영향 없음 (Open Access tier). 한국은 영향 없음. 자세한 내용은 [`protocol.md`](protocol.md) §7.3.
13. **데이터셋 비정기 확장 가능.** OASIS-3는 retrospective compilation이라 신규 환자 모집은 없으나, 기존 환자 대상 새 모달리티/derivative가 비정기적으로 통합됨 (예: 2025-12 Tau sub-project 본 프로젝트 통합 → N 1,378 → 1,379). 분석 시 `accessed YYYY-MM-DD + downloaded subject list snapshot` 보관 권장.
