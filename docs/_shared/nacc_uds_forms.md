# NACC UDS 폼 컬럼 참조 (shared)

> **이 문서는 NACC UDS (Uniform Data Set) 가 정의한 표준이며 NACC / OASIS3 양쪽 코호트에서 참조된다.**
> 각 코호트가 제공하는 파일명·행 수·visit/session 키 변형은 cohort 폴더의 overlay 문서를 참조:
> - **NACC**: [`docs/nacc/uds_forms.md`](../nacc/uds_forms.md), [`docs/nacc/merged_csv.md`](../nacc/merged_csv.md)
> - **OASIS3**: [`docs/oasis3/uds_forms.md`](../oasis3/uds_forms.md)

NACC가 정의하고 모든 ADRC가 매년 시행하는 17개 UDS 폼 (A1–D2) 의 컬럼 그룹별 요약 + 핵심 컬럼 정의. 폼 단위로 1섹션 = 1폼.
...
> **공통 visit-level prefix 컬럼** (모든 UDS 폼에 공통):
> - NACC: `NACCID`, `NACCADC`, `PACKET`, `FORMVER`, `NACCVNUM`, `VISITMO/DAY/YR` — 자세히 [`docs/nacc/uds_forms.md`](../nacc/uds_forms.md)
> - OASIS3: `OASISID`, `OASIS_session_label`, `days_to_visit`, `age at visit` — 자세히 [`docs/oasis3/session_label_reference.md`](../oasis3/session_label_reference.md)
>
> 본 문서의 컬럼 표는 위 prefix 컬럼을 제외한 form-specific 컬럼만 다룬다.

---

## A1 — 참가자 인구통계 (visit-level)

방문마다 **변동 가능**한 인구통계 정보 (cross-sectional baseline 인구통계와 보완 관계).
...
**가장 컬럼 수가 많은 폼** (~398 columns). 가족 구성원별 dementia history를 매트릭스 형태로 수집. Optional module이므로 일부 visit에만 존재.
...
>
> 친조부모 컬럼을 따로 만든 NACC v3.1+ 폼이 있지만 일부 cohort 데이터에는 미포함.

### 활용 예: dementia 가족력 binary flag

```python
import pandas as pd
a3 = pd.read_csv("...UDSa3.csv")
...
> **주의**: 두 파일은 NACC visit/session key로 구별 불가. 분석 시 파일별로 따로 읽고 KEY로 join.
...
NACC UDS의 가장 핵심적인 임상 폼 중 하나. CDR + MMSE + 임상의 1차 진단(`dx1-dx5`)을 묶어서 제공.
...
| CDR Global | CDRTOT (또는 CDRGLOB) | 0 / 0.5 / 1 / 2 / 3 (Hughes 1982 알고리즘) |
...
NACC UDS의 가장 풍부한 진단 정보 폼. 임상 진단 + etiology breakdown + (v3) syndrome flags + biomarker contributory flags.
...
# (cohort에 따라 visit 정렬 키가 다름 — NACC: NACCVNUM, OASIS3: days_to_visit 사용)
# 가장 최근 visit
final_dx = (d1.sort_values(VISIT_KEY)
              .drop_duplicates(SUBJECT_KEY, keep='last'))
...
- **UDS v3 Forms 공식 페이지** (form-by-form PDF): https://www.alz.washington.edu/WEB/forms_uds.html
...
> v71 freeze 기준 PDF는 NFS의 `DEMO/Data_Directory/uds3-rdd.pdf` 에 동봉되어 있다 (NACC 코호트만 해당).
...
# NACC 직접 데이터: FORMVER 컬럼으로 확인 (1=v1, 2=v2, 3=v3, 4=v4)
```
...
## Cohort-specific overlay 문서

NACC 표준 외에 cohort별 quirks (파일명, 행 수, session label 형식, 데이터 입력 오류 등) 는 각 cohort 폴더에서 다룬다:

| Cohort | overlay 문서 | 주요 quirk |
|--------|------------|----------|
| NACC | [`docs/nacc/uds_forms.md`](../nacc/uds_forms.md) | NACC bookkeeping (NACCID/NACCADC/PACKET/FORMVER), v3↔v4 deltas |
| NACC | [`docs/nacc/merged_csv.md`](../nacc/merged_csv.md) | NeuroXT-built merged.csv 390-col 사전 |
| OASIS3 | [`docs/oasis3/uds_forms.md`](../oasis3/uds_forms.md) | OASIS3 file paths + row counts + session tokens (USDa3 typo, c1=psychometrics) |
| OASIS3 | [`docs/oasis3/session_label_reference.md`](../oasis3/session_label_reference.md) | OASISID + days_to_visit + 음수 days 5건 |
