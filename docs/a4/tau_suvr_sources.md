...
| **imaging_SUVR_tau.csv** | `DEMO/Clinical/External Data/` | **Long** | 296,679행 (454명, 최대 8 visits, 192 regions) | Avid/Clark | Whole cerebellum (GM+WM) |
...
| **Atlas** | AAL + FreeSurfer VOI 혼합 (template) | FreeSurfer aparc+aseg (native) | FreeSurfer gtmseg (native) |
...
| **Target ROI 수** | 192 regions (6 composite + 186 regional) | All aparc+aseg (~273) | All aparc+aseg (~243) |
...
**처리 단계:**
1. FreeSurfer `recon-all` → `nu.mgz`, `aparc+aseg.mgz`
...
[Clark et al., Lancet Neurol 2012](https://doi.org/10.1016/S1474-4422(12)70142-4);
[Joshi et al., J Nucl Med 2015](https://doi.org/10.2967/jnumed.114.153494).
...
**SUVR 컬럼 (tau에서 실제 사용 가능한 것):**
- `suvr_cer` — whole cerebellum reference (**tau 데이터에서는 전체 NaN**, amyloid 전용)
- `suvr_persi` — **inferior cerebellar GM reference** (SUIT template indices 6, 8-28, 33, 34) ✓ tau 사용 가능
- `suvr_crus` — **cerebellar crus reference** ✓ tau 사용 가능

> **주의**: tau 분석 시 `suvr_cer`는 비어있으므로 `suvr_persi` 또는 `suvr_crus`를 사용해야 함.
...
`imaging_SUVR_tau.csv` 직접 사용. 최대 8개 시점, 192 ROI. `suvr_persi` 또는 `suvr_crus` 사용 (`suvr_cer`는 tau에서 비어있음).
...
| Avid/Clark method | [Clark et al., Lancet Neurol 2012](https://doi.org/10.1016/S1474-4422(12)70142-4) |
| Avid/Clark ROI | [Joshi et al., J Nucl Med 2015](https://doi.org/10.2967/jnumed.114.153494) |
