...
def build_mriqc(rda_dir: str, output_dir: str, date_str: str = None):
    """Build MRIQC CSV from .rda files (port of 08_extract_mriqc_csv.R)."""
    if date_str is None:
        date_str = datetime.now().strftime('%y%m%d')

    logger.info('Building MRIQC CSV...')

    MRIQC = load_rda(os.path.join(rda_dir, 'MRIQC.rda'))
    REGISTRY = load_rda(os.path.join(rda_dir, 'REGISTRY.rda'))

    if MRIQC is None or REGISTRY is None:
        logger.error('Could not load MRIQC or REGISTRY')
        return None

    logger.info('  - MRIQC: %d records', len(MRIQC))

    # PTID -> RID mapping
    ptid_rid = REGISTRY[['PTID', 'RID']].drop_duplicates()

    # Rename ParticipantID -> PTID
    if 'ParticipantID' in MRIQC.columns:
        MRIQC = MRIQC.rename(columns={'ParticipantID': 'PTID'})

    mriqc_export = MRIQC.merge(ptid_rid, on='PTID', how='left')

    # Standardize VISCODE
    if 'VISCODE2' in mriqc_export.columns:
        mriqc_export['VISCODE'] = mriqc_export['VISCODE2'].apply(
            lambda v: 'bl' if v in ('sc', 'scmri') else v
        )
    if 'StudyDate' in mriqc_export.columns:
        mriqc_export['EXAMDATE'] = pd.to_datetime(mriqc_export['StudyDate'], errors='coerce')

    # Reorder columns
    key_cols = ['RID', 'PTID', 'VISCODE', 'EXAMDATE',
                'SeriesType', 'SeriesDescription',
                'MagneticFieldStrength', 'ScannerManufacturer', 'ScannerModel',
                'SliceThickness', 'NumberVolumes', 'SlicesPerVolume',
                'AcquisitionType', 'AcquisitionPlane',
                'LONIStudy', 'LONISeries', 'LONIImage']
    key_cols = [c for c in key_cols if c in mriqc_export.columns]
    other_cols = [c for c in mriqc_export.columns if c not in key_cols
                  and c not in ('VISCODE2', 'StudyDate')]
    mriqc_export = mriqc_export[key_cols + other_cols]

    # Sort
    sort_cols = [c for c in ['RID', 'VISCODE', 'SeriesType', 'EXAMDATE'] if c in mriqc_export.columns]
    mriqc_export = mriqc_export.sort_values(sort_cols).reset_index(drop=True)

    # Save
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f'MRIQC_{date_str}.csv')
    mriqc_export.to_csv(output_file, index=False, na_rep='')

    logger.info('  - MRIQC saved: %s (%d rows, %d cols)',
                output_file, len(mriqc_export), len(mriqc_export.columns))
    return mriqc_export


def build_apoeres(rda_dir: str, output_dir: str, date_str: str = None):
    """Build APOERES CSV from .rda files (port of 09_extract_apoeres_csv.R)."""
    if date_str is None:
        date_str = datetime.now().strftime('%y%m%d')

    logger.info('Building APOERES CSV...')

    APOERES = load_rda(os.path.join(rda_dir, 'APOERES.rda'))
    REGISTRY = load_rda(os.path.join(rda_dir, 'REGISTRY.rda'))

    if APOERES is None or REGISTRY is None:
        logger.error('Could not load APOERES or REGISTRY')
        return None

    logger.info('  - APOERES: %d records', len(APOERES))

    # RID -> PTID mapping (drop PTID from APOERES if it exists to avoid _x/_y suffix)
    rid_ptid = REGISTRY[['RID', 'PTID']].drop_duplicates()
    apoeres_clean = APOERES.drop(columns=['PTID'], errors='ignore')

    apoeres_export = apoeres_clean.merge(rid_ptid, on='RID', how='left')

    # Reorder columns
    key_cols = ['RID', 'PTID', 'VISCODE', 'EXAMDATE', 'GENOTYPE', 'APGEN1', 'APGEN2']
    key_cols = [c for c in key_cols if c in apoeres_export.columns]
    other_cols = [c for c in apoeres_export.columns if c not in key_cols]
    apoeres_export = apoeres_export[key_cols + other_cols]

    apoeres_export = apoeres_export.sort_values('RID').reset_index(drop=True)

    # Save
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f'APOERES_{date_str}.csv')
    apoeres_export.to_csv(output_file, index=False, na_rep='')

    logger.info('  - APOERES saved: %s (%d rows, %d cols)',
                output_file, len(apoeres_export), len(apoeres_export.columns))
    return apoeres_export
